...
class JurusanController extends Controller
{
	...
	{
		$this->column_title = array('Jurusan', 'Dibuat Pada', 'Dibuat Oleh');
		$this->ajax_field = array(['jurusan', 'created_at', 'created_by'], [0]);
		$this->validation = array(
			'jurusan' => 'required|string|max:191',
		);
		$this->create_form = array(
			'Jurusan' => Form::text('jurusan', old('jurusan'), ['class' => 'form-control', 'placeholder' => 'Contoh: Teknik Informatika', ] ),
		);
		$this->update_form = array(
			'Jurusan' => Form::text('jurusan', old('jurusan'), ['class' => 'form-control', 'placeholder' => 'Contoh: Teknik Informatika', 'id' => 'jurusan'] ),
		);
	}
	...
	public function getData(Datatables $datatables, Request $request)
	{
		...
		$data =  Jurusan::leftJoin('users', 'jurusan.created_by', 'users.id')
						->get([
	...
	function postCreate(Request $request)
	{
		...
		$jurusan->created_by = Auth::user()->id;
	...
	{
		return $data['jurusan'] = Jurusan::find($id)->toJson();
	...
	public function postUpdate(Request $request)
	{
		...
		$jurusan = Jurusan::find($request->input('edit_id'));

		$jurusan->jurusan = $request->input('jurusan');
		$jurusan->updated_by = Auth::user()->id;
	...
	public function delete($id)
	{
		...
		$jurusan->deleted_by = Auth::user()->id;
		$jurusan->save();
