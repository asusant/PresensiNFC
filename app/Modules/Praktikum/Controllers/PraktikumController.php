...
class PraktikumController extends Controller
{
	...
	public function index()
	{
		...
		$data['upload_route'] = route($this->slug.'.aktif.post.upload.create');
		 ...
		 if ($extension == "xlsx" || $extension == "xls" || $extension == "csv") {
			...
			if(!empty($data) && $data->count()){
				 foreach ($data as $key => $value) {
					if ($this->cekJadwal((int) number_format($value->laboratorium,0),(int) number_format($value->hari,0), date("H:i:s", strtotime($value->mulai)), date("H:i:s", strtotime($value->selesai)))->count() > 0) {
	 			        return redirect()->back()->with('message_danger', 'Jadwal '.$value->praktikum.' gagal ditambahkan! Jadwal tumbukan dengan jadwal yang lain!');
	 				}
				 }
				 ...
				 if($bool){
					...
					foreach ($data as $key => $value) {
		 				...
		 				$praktikum->praktikum = $value->praktikum;
		 				$praktikum->id_laboratorium = $value->laboratorium;
		 				$praktikum->id_user_dosen = $value->dosen;
		 				$praktikum->id_hari = $value->hari;
		 				$praktikum->waktu_mulai = $value->mulai;
		 				$praktikum->waktu_selesai = $value->selesai;
		 				$praktikum->created_by = Auth::user()->id;
		 				$praktikum->save();

						Log::aktivitas('Menambah praktikum "'.$value->praktikum.'." dengan import excel.');
				 }
			}
			return redirect()->back()->with('message_sukses', 'Jadwal berhasil ditambahkan.');
		 }else {
	...
	function postCreate(Request $request)
	{
		...
		$praktikum->waktu_mulai = date("H:i:s", strtotime($request->input('waktu_mulai')));
		$praktikum->waktu_selesai = date("H:i:s", strtotime($request->input('waktu_selesai')));
	...
	{
		return $data['praktikum'] = Praktikum::where('id', $id)->first([
												'id',
												'praktikum',
